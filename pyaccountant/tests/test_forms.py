...
from pyaccountant.models import Account, InternalAccountType, Transaction, TransactionJournal
...
class FormTests(TestCase):
    ...
    def test_DepositForm(self):
        ...
        for i in range(1, 3):
            ...
            self.assertEquals(len(Account.objects.filter(
                internal_type=InternalAccountType.revenue.value)), 1)
    ...
    def test_WithdrawForm(self):
        ...
        for i in range(1, 3):
            ...
            self.assertEquals(len(Account.objects.all()), 3)
            self.assertEquals(len(Account.objects.filter(
                internal_type=InternalAccountType.expense.value)), 1)
            new_account = Account.objects.get(
    ...
    def test_different_revenue_accounts(self):
        ...
        form.save()
        self.assertEquals(len(Account.objects.filter(
            internal_type=InternalAccountType.revenue.value)), 2)
    ...
    def test_different_expense_accounts(self):
        ...
        form.save()
        self.assertEquals(len(Account.objects.filter(
            internal_type=InternalAccountType.expense.value)), 2)
